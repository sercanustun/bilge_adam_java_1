...
public class SinavdaLazim2 {
	...
	private static void findUser() {
		...
		List<Worker> tempList = personList.stream().filter(p -> p.getName().startsWith(searchText))
	...
	private static List<Worker> getUsersFromDatabase() {
	...
	private static Worker castToPerson(String readText) {
	...
	public static void addUser() {
		...
		personList.add(new Worker(userName, password, UUID.randomUUID().toString()));
		writeDatabase(personList);
	}
	
	private static void writeDatabase(List<Worker> listPerson) {
		try (BufferedWriter bW = new BufferedWriter(new FileWriter(PATH))) {
			for (Worker person : listPerson) {
